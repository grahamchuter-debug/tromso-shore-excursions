import schedulePayload from "@/data/tromso-cruise-schedules.generated.json";

export type TromsoScheduleRow = {
  port: string;
  ship: string;
  cruise_line: string;
...
export type TromsoScheduleEntry = {
  date: string;
  ship: string;
  arrival: string;
  departure: string;
...
type SchedulePayload = {
  ...
  rows: TromsoScheduleRow[];
};

const payload = schedulePayload as SchedulePayload;

export const tromsoScheduleIntegrity = payload.integrity;
export const tromsoScheduleMeta = {
  port: payload.port,
...
function toEntry(row: TromsoScheduleRow): TromsoScheduleEntry {
  return {
...
export function getAllTromsoRows(): TromsoScheduleRow[] {
  return payload.rows;
}

export function getTromsoEntries(): TromsoScheduleEntry[] {
  return payload.rows.map(toEntry);
}

export function getTromsoEntriesForMonthKey(monthKey: string): TromsoScheduleEntry[] {
  return payload.rows
...
export function getTromsoMonthKeysWithCalls(): string[] {
  return [...new Set(payload.rows.map((row) => row.arrival_date.slice(0, 7)))].sort();
}

export function getTromsoMonthSummaries() {
...
export const scheduleDisclaimer =
  "Published times come from the Norway Shore Excursions master schedule import. Always confirm arrival, departure and all aboard with your cruise line before finalising plans. A published Tromsø call does not mean aurora or wildlife will be visible.";
